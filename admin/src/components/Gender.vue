...
export default {
    ...
    created() {
        getGenderAjax()
            .then(val => {
                let genderArr = []
                val.data.data.map((item,index) => {//初始化以下val.data.data
                    let obj = {}
                    obj.value = item.count
                    item._id == 'true' ? obj.name = '男生' : obj.name = '女生'
                    genderArr.push(obj)
                })
                this.genderArr = genderArr
                this.setMyEchart()//调用EChart渲染方法
            })
            .catch(err => {
